...
type
  ...
  public
    ...
    procedure PrintSomeWords(wrd:string; items:TTreeNodes; DelChar:char);
...
procedure TNode.PrintSomeWords(wrd:string; items:TTreeNodes; DelChar:char);
var
  ch:TIndex;
begin
  if FPoint then
  ...
  for ch:= lowCh to HighCh do
    if (FNext[ch] <> nil) and (DelChar <> ch) then
      begin
        wrd:=wrd+ch;
        FNext[ch].PrintSomeWords(wrd, items, DelChar);
        SetLength(wrd,length(wrd)-1);
      end;
end;
